...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        #                        padding=3, bias=False)),#修改通道3--改为1（YH）
    ...
    def forward(self, x):
        ...
        #print("self.features(x)=",features.shape)
        out = F.relu(features, inplace=True)
        #print("F.relu(features, inplace=True)=",out.shape)
        out = F.adaptive_avg_pool2d(out, (1, 1))    # 自适应均值池化，输出大小为（1，1）
        #print("F.adaptive_avg_pool2d(out, (1, 1))=",out.shape)
        out = torch.flatten(out, 1)
        #print("torch.flatten(out, 1)=",out.shape)
        out = self.classifier(out)  # 分类器
        #print("self.classifier(out)=",out.shape)
        return out
